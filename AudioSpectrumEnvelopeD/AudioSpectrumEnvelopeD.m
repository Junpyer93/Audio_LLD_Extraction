...
% se la struttura è vuota il codice va con i valori di default
...
%nargin restituisce il numero di argomenti passati all'interno della
%funzione

if nargin<4 writeXML=0;end                                                 %Se non esplicitata la scrittura in file XML
if nargin<2 hopSize= 'PT07N1000F';  end; %'PT10N1000F'; 
%poichè non abbiamo passato il valore dell'intervallo di tempo tra due time frames successivi
%PT10N1000F E' un formato 

% Read in audio file

% wavread e auread sono stati sostituiti da audioread nelle ultime versioni
% di Matlab 
...
if size(audioData,2)>1  %la funzione size restituisce la dimensione delle colonne di audioData, nei file .wav è il numero dei canali
...
%hop = [str2num(hopSize(3:i-1))/str2num(hopSize(i+1:end-1))]; %10 1000
standvar = h_mpeg7init(sr,hop);

%standvar crea una struttura con windowside=30ms e hopsize=10ms
...
try
    if isempty(attributegrp.hiEdge) %isempty ritorna 1 se l'array è vuoto
        attributegrp.hiEdge= 16000; %valore di default
    end
catch
...
AudioSpectrumEnvelope = []; %vettore vuoto
